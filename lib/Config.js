...
function azureLocationConstraintAssert(location, locationObj) {
    ...
    assert(storageAccountNameFromEnv || azureStorageAccountName,
        `bad location constraint: "${location}" azureStorageAccountName must ` +
        'be set in locationConfig or environment variable');
    assert(storageAccessKeyFromEnv || azureStorageAccessKey,
        `bad location constraint: "${location}" azureStorageAccessKey must ` +
        'be set in locationConfig or environment variable');
    [storageAccountNameFromEnv, azureStorageAccountName].forEach(value => {
        if (value !== undefined) {
            assert.strictEqual(typeof value, 'string',
            `bad location constraint: "${location}" ` +
            `azureStorageAccountName "${value}" must be a string`);
            assert(azureAccountNameRegex.test(value),
                `bad location constraint: "${location}" ` +
                `azureStorageAccountName "${value}" is an invalid value`);
        }
    });
    [storageAccessKeyFromEnv, azureStorageAccessKey].forEach(value => {
        if (value !== undefined) {
            assert.strictEqual(typeof value, 'string',
            `bad location constraint: "${location}" ` +
            `azureStorageAccessKey "${value}" must be a string`);
            assert(azureAccessKeyRegex.test(value),
                `bad location constraint: "${location}" ` +
