...
const { azureAccountNameRegex, azureAccessKeyRegex } = require('../constants');
...
function azureLocationConstraintAssert(location, locationObj) {
    ...
    const { azureStorageAccountName, azureStorageAccessKey,
        azureContainerName } = locationObj.details;
    const storageAccountName =
        storageAccountNameFromEnv || azureStorageAccountName;
    const storageAccessKey =
        storageAccessKeyFromEnv || azureStorageAccessKey;
    assert(storageAccountName, 'bad location constraint: ' +
        `"${location}" azureStorageAccountName must be ` +
        'set in locationConfig or environment variable');
    assert(storageAccessKey, 'bad location constraint: ' +
        `"${location}" azureStorageAccessKey must be ` +
        'set in locationConfig or environment variable');
    assert.strictEqual(typeof storageAccountName, 'string',
        `bad location constraint: "${location}" azureStorageAccountName ` +
        `"${storageAccountName}" must be a string`);
    assert(azureAccountNameRegex.test(storageAccountName),
        `bad location constraint: "${location}" ` +
        `azureStorageAccountName "${storageAccountName}" is ` +
        'an invalid value');
    assert.strictEqual(typeof storageAccessKey, 'string',
        `bad location constraint: "${location}" azureStorageAccessKey ` +
        `"${storageAccessKey}" must be a string`);
    assert(azureAccessKeyRegex.test(storageAccessKey),
        `bad location constraint: "${location}" ` +
        'azureStorageAccessKey is not a valid base64 string');
    assert(azureContainerName !== undefined, 'bad location constraint: ' +
        `"${location}" azureContainerName must be defined`);
    assert.strictEqual(typeof azureContainerName, 'string',
        `bad location constraint: "${location}" azureContainerName ' +
        'must be a string`);
...
class Config extends EventEmitter {
    ...
    isSameAzureAccount(locationConstraintSrc, locationConstraintDest) {
        ...
        .map(locationConstraint =>
            this.getAzureStorageAccountName(locationConstraint));
        return accountNames[0] === accountNames[1];
    }
